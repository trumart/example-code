...
class FinanceService extends BaseService
{
    ...
    public function remove(int $financeId): JsonResponse
    {
        ...
        if (!$delete) {
            return apiError('Не получилось удалить фин. операцию');
        }
    ...
    public function broadcast(array $inp): Finance|JsonResponse
    {
        ...
        if (empty($checkUser)) {
            return apiError('Воу, Воу, Погоди твоя смена закрыта, я не могу разрешить данное действие');
        }
        ...
        foreach ($inp['finances']['data'] as $item) {
            ...
            if ($item['check'] == true && !empty($item['distribution'])) {
                return apiError('По платежу ' . $item['title'] . ' от ' . date('d.m.Y', strtotime($item['date'])) . ' на сумму ' . $item['sum'] . ' уже было распределение');
            }
        ...
        if ($inp['broadcast']['sum'] > $paymentsSum) {
            return apiError("Сумма передачи д/c {$inp['broadcast']['sum']} больше чем сумма {$paymentsSum} по выбранным платежам");
        }
    ...
    public function getInn(array $inp): int|null|JsonResponse
    {
        ...
        if (!empty($inp['inn']) && is_numeric($inp['inn'])) {
            ...
            if ($innLen !== 10 && $innLen !== 12) {
                return apiError('ИНН должно содержать или 10 или 12 цифр');
            }
