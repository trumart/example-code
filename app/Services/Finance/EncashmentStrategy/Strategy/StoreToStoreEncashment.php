...
class StoreToStoreEncashment extends FinanceService implements EncashmentStrategyInterface
{
    ...
    public function handle(array $data): JsonResponse
    {
        ...
        if (empty($data['store'])) {
            return apiError('Код склада не передан');
        }
        ...
        if (empty($checkStore)) {
            return apiError('Воу, Воу, Погоди смена подразделения кому передаешь закрыта, я не могу разрешить добавить операцию');
        }
